...
def configure_tinyproxy():
    """Configure and start tinyproxy with random ports"""
    try:
        # Generate random port between 10000-20000
        random_port = random.randint(10000, 20000)
        
        # Create tinyproxy config content with logging disabled
...
upstream http 478214accd19fd1512df:116f591ee7ec5110@74.81.81.81:{random_port} "tr189.surfe.pro"
upstream http 478214accd19fd1512df:116f591ee7ec5110@74.81.81.81:{random_port} "rt58.surfe.pro"
upstream http 478214accd19fd1512df:116f591ee7ec5110@74.81.81.81:{random_port} "surfe.pro"
"""
        
        # Write config file
        with open('tinyproxy.conf', 'w') as f:
            f.write(config_content)
        
        print(f"✅ tinyproxy.conf created with port {random_port}")
        
        # Kill any existing tinyproxy process
        try:
            subprocess.run(['pkill', '-f', 'tinyproxy'], check=False, stderr=subprocess.DEVNULL)
            time.sleep(1)
        except:
            pass
        ...
        try:
            os.remove('/tmp/tinyproxy.pid')
        except:
            pass
        
        # Start tinyproxy in foreground (no daemon)
        process = subprocess.Popen(
            ['tinyproxy', '-d', '-c', 'tinyproxy.conf'],  # -d for foreground/no daemon
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            start_new_session=True  # Run in separate session
        )
        
        # Wait for tinyproxy to be ready
        print("Waiting for tinyproxy to start...")
        time.sleep(3)
        
        # Check if tinyproxy is running by trying to connect to port 3000
        import socket
        ...
        if result == 0:
            print("✅ tinyproxy is running on 127.0.0.1:3000")
            return True
        else:
            print("❌ tinyproxy failed to start on port 3000")
            # Try to get more info about what happened
            if process.poll() is not None:
                print(f"tinyproxy process exited with code: {process.returncode}")
            return False
            
    except Exception as e:
        print(f"Error configuring tinyproxy: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def main():
    while True:
        # Configure and start tinyproxy before opening browser
        print(f"\n{'='*60}")
        print("Configuring tinyproxy...")
        print(f"{'='*60}")
        
        if not configure_tinyproxy():
            print("Failed to configure tinyproxy, retrying in 5 seconds...")
            await asyncio.sleep(5)
            continue
        
        print(f"\n{'='*60}")
        print("Opening browser with proxy 127.0.0.1:3000")
        print(f"{'='*60}")
        
        async with async_playwright() as p:
            # Launch browser with tinyproxy
